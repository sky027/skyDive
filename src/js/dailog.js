...
var dialog = {
  ...
  setstyle: function(data){
    ...
    var html = '<div class="dailog'+time+'">'
        +'<div id="dailogover" class="dailogover" style="width: '+data.width+'px;height: '+data.height+'px"></div></div>';
    $("body").append(html);
    var dom = $('dailog'+ time);
    ...
    dom.find('.dailogover');
    import comp from '@/components/common/combtn.vue';
    var vms = new Vue({
      el: '#dailogover',
      /* components: resolve => require(['@/components/common/combtn.vue'], resolve), */
      components: comp
    });
